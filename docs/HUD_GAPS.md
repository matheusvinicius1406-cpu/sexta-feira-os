...
| `cortex` | **novo router — o cérebro simbólico (sem LLM)** — `POST /cortex/intent` (fala → intenção → ação nas engines reais, com `trace` da decisão e resposta honesta) + `GET /cortex/verbs` (a gramática que ele conhece). A paleta consulta o cortex **antes** do chat: texto que nenhum comando dinâmico casa vira `Jarvis: <fala>` → painel Cortex mostra verbo/alvo/resposta/trilha; `falar <texto>` sobe o áudio com a voz do pack ativo; `understood=false` lista o que ele sabe em vez de inventar. **Mic na paleta** (`#palMic`, Web Speech pt-BR): falou → transcreve → executa sozinho, mesmo caminho do teclado. **Camada de regras declarativas**: `backend-core/rules/*.yaml` (condição → ação, sem código arbitrário; `auto` controla propor vs executar) + `GET /cortex/rules` e `POST /cortex/rules/avaliar` (monta o snapshot do mundo e devolve decisões com a trilha condição por condição — o "por que decidi"); paleta `avaliar regras` e `regras` → painel Cortex · Regras. **Operadores**: hora/dia, rádio, voz, CPU, metas, memória total + **mundo** (`fato_igual`, `fato_existe`), **marcadores** (`marcador_existe`, `marcadores_maior_que`), **timer** (`timer_rodando`, `timer_label`), **memória semântica** (`memoria_tem` sobre as recentes) + `metas_ativas_menor_que`, `briefing_hoje` (report do dia já gerado?). **Rotinas reais** (`rules/rotinas.yaml`, 13 regras): trabalho (início/fim de expediente por `localizacao`+timer, reunião de manhã, foco sem meta), rotina (briefing pendente, fim de semana, domingo de planejamento, noite de revisão, casa no fim de tarde) e saúde (energia baixa, madrugada acordado, dormindo com música, acordou cedo) — cada uma com a trilha do porquê. **Núcleo decisório** (`POST /cortex/decidir`): um ciclo único sem LLM — contexto → regras → metas ranqueadas pelo DecisionEngine (`rank_goals`, sem persistir) → escolha determinística (regra forte ≥ p20 vence o foco; senão a meta melhor rankeada; senão "nenhuma") com rationale + trilha, persistida em Decision (question="nucleo", auditável em `decision/history`; `GET /cortex/decidir/ultimo`); paleta `decidir` → painel Cortex · Decisão. **Regras → pulse**: `POST /cortex/regras/propor` transforma cada regra disparada em proposta do agente (source="cortex") com a trilha no reason; aprovar com 1 clique (`▶ executar` em Agents · Proposals, ou paleta `aprovar <id>`) executa a ação pela tool `cortex_regra` (intent roda nas engines; sugestao/observar viram aprendizado; falar devolve o texto); `auto: true` executa na hora pelo mesmo rastro; dedupe impede proposta repetida. **Ciclo autônomo agendado**: `POST /cortex/ciclo/agendar` (idempotente — tarefa recorrente kind="cortex", padrão a cada 60 min, via scheduler do kernel) e `GET /cortex/ciclo` (status: agendado + histórico de execuções + propostas de regra). No tick, o kernel acorda sozinho e chama `propor_regras` com o contexto real (proposer injetado no step `agent`): regra disparada vira proposta sem ninguém digitar; dedupe e `auto: true` valem igual. Paleta `ciclo` (status) e `agendar ciclo` (a cada hora) → painel Cortex · Ciclo — **2026-08-15** |
