...
| `voice` | `voice/*` + loop de voz (`live.js`) + **packs falam com voz TTS real** — cada pack carrega `tts_voice` (Edge neural: Jarvis Classic→Antonio, Amigável→Francisca), `tts_rate` e `tts_pitch`; `usar voz <nome>` reconfigura o synthesizer (muda o som, não só as frases); paleta `falar <texto>` faz o Jarvis dizer o que você mandar via `/voice/speak` — **2026-08-15** |
